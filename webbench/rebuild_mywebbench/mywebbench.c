...
int mypipe[2];   // 管道，用于父子进程通信

//往管道里面写到内容
unsigned int speed=0; // 记录进程成功得到服务器相应的数量，即成功数
unsigned int failed=0; // 记录失败的数目
unsigned int bytes=0;  // 记录进程成功读取的字节数，当force = 0时有效

// volatile 这个关键字是一个很有意思的东西
    ...
    else  //父进程
    {
        ...
        */
        ...
        while(1){
		    ...
		    if(pid<2)
            {
                printf("Some of our childrens died.\n");
                break;
            }
