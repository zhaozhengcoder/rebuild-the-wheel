...
struct ngx_module_s {
    ngx_uint_t            ctx_index;                        /*分类的模块计数器  nginx模块可以分为四种：core、event、http和mail 每个模块都会各自计数，ctx_index就是每个模块在其所属类组的计数*/
    ngx_uint_t            index;                            /*一个模块计数器，按照每个模块在ngx_modules[]数组中的声明顺序，从0开始依次给每个模块赋值*/
    ...
    ngx_command_t        *commands;                         /*命令定义地址 模块的指令集 每一个指令在源码中对应着一个ngx_command_t结构变量*/
